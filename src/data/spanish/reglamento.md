...
| --- | --- |
| Choque / Élite (infantería) | 25 – 40 mm redonda |
...
- **Modo Escaramuza:** cada miniatura actúa como una unidad independiente. Es un modo más táctico y narrativo, ideal para partidas rápidas y mesas pequeñas. **En Escaramuza no se pueden incluir Vehículos, Monstruos ni Artillería** — estos tipos de unidad están reservados para el modo Gran Batalla.
- **Modo Gran Batalla:** las unidades se agrupan en escuadras que actúan como una sola entidad. Es el modo pensado para batallas de mayor escala, donde todos los tipos de unidad están disponibles, incluidos Vehículos, Monstruos y Artillería.
...
- Realizar hasta **dos acciones**, respetando el coste de cada una.
...
1. Elegir objetivo.
2. Determinar el número de dados de Ataques del arma.
...
En Gran Batalla, cada miniatura utiliza las armas que tenga equipadas.
...
Los **Vehículos y Monstruos** no se benefician de cobertura. Sus dimensiones impiden que cualquier obstáculo les proporcione protección efectiva, salvo que la unidad esté completamente fuera de la línea de visión.
...
| Tipo | Descripción | Reglas | Fuerte contra |
| --- | --- | --- | --- |
...
![Ejemplo visual de perfil de unidad con clase, set, valor y estadísticas]({{unitProfileImage}})
...
Incluye comportamientos especiales listados abajo; estas reglas se aplican siempre que la unidad esté en juego, salvo que se indique lo contrario. La habilidad depende de la **clase** de la unidad y del **set** elegido, según la siguiente tabla. Los Héroes tienen su propia habilidad especial, indicada en su ficha.

| Clase | Equilibrado | Disparo | Asalto |
| --- | --- | --- | --- |
...
Indica el **número mínimo y máximo de miniaturas** que puede incluir la unidad cuando se juega en modo Gran Batalla. Este valor no se utiliza en modo Escaramuza.
...
Cada unidad lleva exactamente **2 armas** en su perfil: **una a distancia y una de cuerpo a cuerpo**, definidas por el set elegido. Al realizar un ataque, el jugador utiliza el arma correspondiente al tipo de ataque. No es posible usar ambas armas en el mismo ataque.

### Sets de armas

Cada unidad dispone de tres **sets de armas** que definen su estilo de combate repartiendo la potencia entre sus dos armas. El set no cambia las estadísticas base ni el rol de la unidad, solo su armamento. Cada unidad indica en su ficha qué armas componen cada uno de sus sets. Al montar la lista, para cada unidad se elige uno de estos tres sets:

- **Asalto:** el arma de **cuerpo a cuerpo es la potente**; el arma a distancia es un apoyo menor. Para unidades que buscan el combate cercano.
- **Equilibrado:** ambas armas están **equiparadas** a un nivel medio. Flexible, sin picos de potencia.
- **Disparo:** el arma **a distancia es la potente**; el arma de cuerpo a cuerpo es una defensa mínima. Para unidades volcadas al fuego a distancia.

El set elegido se mantiene durante toda la partida. Como variante opcional, los jugadores pueden acordar que toda la partida se juegue con un **único set** para todas las unidades de ambos ejércitos.

## Perfiles de armas a distancia

Las armas a distancia se describen mediante su propio perfil. Cada arma indica cómo se resuelve su ataque cuando es utilizada. Un perfil de arma a distancia incluye:
